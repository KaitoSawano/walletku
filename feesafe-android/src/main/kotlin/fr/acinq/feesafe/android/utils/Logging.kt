...
object Logging {
    ...
    private const val CURRENT_LOG_FILE = "feesafe.log"
    private const val ARCHIVED_LOG_FILE = "feesafe.archive-%i.log"

    fun exportLogFile(context: Context): File {
        val export = File(File(context.cacheDir, LOGS_EXPORT_DIR).also { if (!it.exists()) it.mkdir() },
            "feesafe_logs-${UUID.randomUUID().toString().substringBefore("-")}.log")
        ...
        File(File(context.filesDir, LOGS_DIR), "feesafe.archive-1.log").takeIf {
    ...
    fun setupLogger(context: Context) {
        // cleanup export file
        val export = File(File(context.cacheDir, LOGS_DIR), "feesafe_export.log")
        ...
        lc.getLogger("fr.acinq.feesafe").level = if (BuildConfig.DEBUG) Level.DEBUG else Level.INFO
