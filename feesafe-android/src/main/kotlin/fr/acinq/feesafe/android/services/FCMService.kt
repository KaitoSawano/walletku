...
class FCMService : FirebaseMessagingService() {
        ...
        when {
            ...
            remoteMessage.priority != RemoteMessage.PRIORITY_HIGH -> {
                ...
                serviceScope.launch {
                    ...
                    val walletMetadata = (application as FeeSafeApplication).globalPrefs.getAvailableWalletsMeta.first().getByWalletIdOrDefault(walletId)
                    when (reason) {
                ...
                log.debug("starting feesafe foreground service with reason=$reason")
                startFeeSafeForegroundService(reason, nodeIdHash)
            }
        }
    }

    private fun startFeeSafeForegroundService(reason: String?, nodeIdHash: String) {
    ...
    override fun onNewToken(token: String) {
        serviceScope.launch(Dispatchers.IO + CoroutineExceptionHandler { _, e ->
            ...
            val globalPrefs = (applicationContext as FeeSafeApplication).globalPrefs
