...
package fr.acinq.feesafe.android.navigation
...
import fr.acinq.feesafe.android.AppViewModel
...
import fr.acinq.feesafe.android.payments.history.PaymentsExportView
import fr.acinq.feesafe.android.payments.history.PaymentsHistoryView
import fr.acinq.feesafe.android.payments.send.liquidity.RequestLiquidityView
import fr.acinq.feesafe.android.settings.AboutView
import fr.acinq.feesafe.android.settings.AppAccessSettings
import fr.acinq.feesafe.android.settings.troubleshoot.TroubleshootingView
import fr.acinq.feesafe.android.settings.DisplayPrefsView
import fr.acinq.feesafe.android.settings.ExperimentalView
import fr.acinq.feesafe.android.settings.ForceCloseView
import fr.acinq.feesafe.android.settings.NotificationsView
import fr.acinq.feesafe.android.settings.PaymentSettingsView
import fr.acinq.feesafe.android.settings.reset.ResetWallet
import fr.acinq.feesafe.android.settings.SettingsContactsView
import fr.acinq.feesafe.android.settings.SettingsView
import fr.acinq.feesafe.android.settings.TorConfigView
import fr.acinq.feesafe.android.settings.displayseed.DisplaySeedView
import fr.acinq.feesafe.android.settings.electrum.ElectrumView
import fr.acinq.feesafe.android.settings.fees.AdvancedIncomingFeePolicy
import fr.acinq.feesafe.android.settings.fees.LiquidityPolicyView
...
fun NavGraphBuilder.settingsNavGraph(navController: NavController, appViewModel: AppViewModel) {
    ...
    businessComposable(Screen.BusinessNavGraph.Notifications.route, appViewModel, deepLinks = listOf(navDeepLink { uriPattern = "feesafe:notifications/{walletId}" })) { backStackEntry, walletId, business ->
    ...
    businessComposable(Screen.BusinessNavGraph.LiquidityPolicy.route, appViewModel, deepLinks = listOf(navDeepLink { uriPattern = "feesafe:liquiditypolicy" })) { _, _, business ->
        LiquidityPolicyView(
            business = business,
            onBackClick = { navController.popBackStackOrHome() },
    ...
    businessComposable(Screen.BusinessNavGraph.LiquidityRequest.route, appViewModel, deepLinks = listOf(navDeepLink { uriPattern = "feesafe:requestliquidity" })) { _, walletId, business ->
