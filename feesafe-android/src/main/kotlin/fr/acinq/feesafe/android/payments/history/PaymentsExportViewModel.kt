...
import fr.acinq.feesafe.android.security.EncryptedData
import fr.acinq.feesafe.android.utils.converters.DateFormatter.toAbsoluteDateTimeString
import fr.acinq.feesafe.csv.WalletPaymentCsvWriter
import fr.acinq.feesafe.managers.DatabaseManager
...
class PaymentsExportViewModel(
    ...
    fun generateCSV(context: Context) {
        viewModelScope.launch(Dispatchers.IO + CoroutineExceptionHandler { _, e ->
            ...
            val file = File.createTempFile("feesafe-", ".csv", exportDir)
    ...
    fun vacuumDatabase(context: Context, uri: Uri) {
        viewModelScope.launch(Dispatchers.IO + CoroutineExceptionHandler { _, e ->
            ...
            val exportDir = File(context.cacheDir, "payments")
            if (!exportDir.exists()) exportDir.mkdir()
            val file = File.createTempFile("feesafe-payments-db-", ".sqlite", exportDir)
