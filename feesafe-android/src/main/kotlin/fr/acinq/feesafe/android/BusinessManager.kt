...
import fr.acinq.feesafe.android.components.wallet.WalletAvatars
import fr.acinq.feesafe.android.services.InflightPaymentsWatcher
...
import fr.acinq.feesafe.android.utils.datastore.UserWalletMetadata
import fr.acinq.feesafe.android.utils.datastore.getByWalletIdOrDefault
import fr.acinq.feesafe.data.StartupParams
import fr.acinq.feesafe.data.inFlightPaymentsCount
import fr.acinq.feesafe.managers.AppConnectionsDaemon
import fr.acinq.feesafe.managers.NodeParamsManager
import fr.acinq.feesafe.managers.PeerManager
...
sealed class StartBusinessResult {
    data class Success(val walletInfo: WalletManager.WalletInfo, val business: FeeSafeBusiness): StartBusinessResult()
...
object BusinessManager {
    ...
    private val application by lazy { appContext as FeeSafeApplication }
    ...
    suspend fun startNewBusiness(words: List<String>, isHeadless: Boolean): StartBusinessResult = startupMutex.withLock {
        ...
        val business = FeeSafeBusiness(application.feesafeGlobal)
        ...
        return try {
            ...
            if (lastVersionUsed == null) {
                // lastUsedAppCode was added in version 99, and is set up during the wallet creation. So if it's null, this FeeSafe was installed prior v99 and we can show a patch note
            ...
            business.feesafeGlobal.currencyManager.startMonitoringCurrencies(walletId = walletId.nodeIdHash, currencies = preferredCurrencies)
            ...
            eventsMonitoringJobs[walletId] = BusinessMonitorJobs(
                monitorHeadlessPaymentsJob = if (isHeadless) {
                    scope.launch { monitorPaymentsWhenHeadless(walletId, walletMetadata, business.nodeParamsManager, application.feesafeGlobal.currencyManager, userPrefs) }
                } else null,
    ...
    private fun doStopBusiness(walletId: WalletId, running: BusinessRunning) {
        ...
        application.feesafeGlobal.currencyManager.stopMonitoringForWallet(walletId.nodeIdHash)
    }
    ...
    private suspend fun monitorFcmToken(business: FeeSafeBusiness) {
