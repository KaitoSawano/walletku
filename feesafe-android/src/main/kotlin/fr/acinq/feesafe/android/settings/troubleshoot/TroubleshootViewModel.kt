package fr.acinq.feesafe.android.settings.troubleshoot
...
import fr.acinq.feesafe.android.utils.Logging.LOGS_EXPORT_DIR
...
import fr.acinq.feesafe.utils.diagnostics.DiagnosticsHelper
...
class DiagnosticsViewModel(val application: FeeSafeApplication, val business: FeeSafeBusiness, val walletId: WalletId) : ViewModel() {
    ...
    fun shareDiagnostics(context: Context) {
        ...
        viewModelScope.launch(Dispatchers.IO + CoroutineExceptionHandler { _, e ->
            ...
            val file = File(File(context.cacheDir, LOGS_EXPORT_DIR).also { if (!it.exists()) it.mkdir() },
                "feesafe_diagnostics-${UUID.randomUUID().toString().substringBefore("-")}.txt")
    ...
    class Factory(
        private val application: FeeSafeApplication,
        private val business: FeeSafeBusiness,
