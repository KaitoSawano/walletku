...
/**
 * Generates a `BuildVersions` file in feesafe-shared/build/generated-src containing the current git commit and the lightning-kmp version.
...
val buildVersionsTask by tasks.registering(Sync::class) {
    from(
        resources.text.fromString(
            """
            |package fr.acinq.feesafe.shared
            |
        ...
        it.binaries {
            ...
            configureEach {
                it.compilations.all {
                    ...
                    // workaround for IndexOutOfBoundsException with feesafe-shared:linkReleaseFrameworkIosArm64 in iOS Archive build
...
sqldelight {
    databases {
        create("ChannelsDatabase") {
            packageName.set("fr.acinq.feesafe.db.sqldelight")
...
if (includeAndroid) {
    extensions.configure<com.android.build.gradle.LibraryExtension>("android") {
        namespace = "fr.acinq.feesafe.shared"
        compileSdk = 34
        defaultConfig {
            minSdk = 26
