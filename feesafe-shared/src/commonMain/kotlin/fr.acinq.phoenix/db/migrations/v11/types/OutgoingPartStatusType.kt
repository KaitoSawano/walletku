...
sealed class OutgoingPartStatusData {
    ...
    sealed class Succeeded : OutgoingPartStatusData() {
        @Serializable
        @SerialName("fr.acinq.feesafe.db.payments.OutgoingPartStatusData.Succeeded.V0")
        data class V0(@Serializable val preimage: ByteVector32) : Succeeded()
    }

    sealed class Failed : OutgoingPartStatusData() {
        @Serializable
        @SerialName("fr.acinq.feesafe.db.payments.OutgoingPartStatusData.Failed.V0")
        data class V0(val remoteFailureCode: Int?, val details: String) : Failed()

        @Serializable
        @SerialName("fr.acinq.feesafe.db.payments.OutgoingPartStatusData.Failed.V1")
