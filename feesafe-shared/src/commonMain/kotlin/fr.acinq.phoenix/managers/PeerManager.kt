...
import fr.acinq.feesafe.managers.global.FeerateManager
...
class PeerManager(
    ...
    constructor(business: FeeSafeBusiness) : this(
        ...
        feerateManager = business.feesafeGlobal.feerateManager,
    ...
    /** Forward compatibility check. [UpgradeRequired] is sent by the peer when an old version of FeeSafe restores a wallet that has been used with new channel types. */
    ...
    val finalWallet = peerState.filterNotNull().flatMapLatest { peer ->
        combine(peer.currentTipFlow.filterNotNull(), peer.feesafeFinalWallet.wallet.walletStateFlow) { currentBlockHeight, wallet ->
    ...
    val swapInWallet = peerState.filterNotNull().flatMapLatest { peer ->
        combine(peer.currentTipFlow.filterNotNull(), peer.channelsFlow, peer.feesafeSwapInWallet.wallet.walletStateFlow) { currentBlockHeight, channels, swapInWallet ->
...
/** The peer's swap-in wallet for FeeSafe is always not null, because the client is always an [IElectrumClient] (see how this Peer is built in `PeerManager.init`). */
val Peer.feesafeSwapInWallet: SwapInWallet
    get() = this.swapInWallet!!

/** The peer's final wallet for FeeSafe is always not null, because the client is always an [IElectrumClient] (see how this Peer is built in `PeerManager.init`). */
val Peer.feesafeFinalWallet: FinalWallet
