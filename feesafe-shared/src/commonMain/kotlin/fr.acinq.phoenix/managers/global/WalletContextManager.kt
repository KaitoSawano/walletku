...
import fr.acinq.feesafe.data.WalletContext
import fr.acinq.feesafe.data.WalletNotice
 ...
 * the service. For example, check what is the latest available version of FeeSafe, or if the
...
class WalletContextManager(
    ...
    private suspend fun fetchWalletContext(): WalletContext? {
        return try {
            httpClient.get("https://acinq.co/feesafe/walletcontext.json")
        } catch (_: Exception) {
            try {
                httpClient.get("https://s3.eu-west-1.amazonaws.com/acinq.co/feesafe/walletcontext.json")
            } catch (e: Exception) {
            ...
            try {
                val base = json[NodeParamsManager.chain.feesafeName]!!
    ...
    private suspend fun fetchWalletNotice(): WalletNotice? {
        return try {
            httpClient.get("https://acinq.co/feesafe/walletnotice.json")
        } catch (_: Exception) {
            try {
                httpClient.get("https://s3.eu-west-1.amazonaws.com/acinq.co/feesafe/walletnotice.json")
            } catch (_: Exception) {
                null
            }
        }?.let { response ->
