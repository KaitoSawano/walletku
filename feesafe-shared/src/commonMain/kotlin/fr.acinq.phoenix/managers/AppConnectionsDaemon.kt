...
class AppConnectionsDaemon(
    ...
    private val feesafeGlobal: FeeSafeGlobal,
    ...
    constructor(business: FeeSafeBusiness) : this(
        ...
        feesafeGlobal = business.feesafeGlobal,
    ...
    init {
        ...
        // Internet monitor
        launch {
            feesafeGlobal.networkMonitor.start()
            feesafeGlobal.networkMonitor.networkState.collect {
                ...
                when {
                    it.internetIsAvailable && it.disconnectCount <= 0 -> {
                        if (!httpControlFlowEnabled) {
                            ...
                            feesafeGlobal.enableNetworkAccess()
                        }
                    }
                    else -> {
                        if (httpControlFlowEnabled) {
                            httpControlFlowEnabled = false
                            feesafeGlobal.disableNetworkAccess()
                        }
                    }
                }
            }
